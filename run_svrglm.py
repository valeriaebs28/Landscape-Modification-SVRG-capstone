...
import time
import sys
from svrglmd import SVRGLMD_k, SVRGLMD_Snapshot
from svrglm import SVRGLM_k, SVRGLM_Snapshot
...
parser = argparse.ArgumentParser(description="Train SVRG/SVRGLM/SVRGLMD on MNIST data.")
parser.add_argument('--optimizer', type=str, default="SVRGLM",
                    help="optimizer.")
...
#modified to fit SVRGLM: train_epoch_SVRG
def train_epoch_SVRGLM(model_k, model_snapshot, optimizer_k, optimizer_snapshot, train_loader, loss_fn, flatten_img=True):
    model_k.train()
    ...
    # pass the current parameters of optimizer_0 to optimizer_k 
    ...
    for images, labels in train_loader:
        ...
        # print("added item in loss.update for train loss:",loss_iter.data.item() ) #added
        acc.update(acc_iter)
...
# modified to fit SVRGLMD
def train_epoch_SVRGLMD(model_k, model_snapshot, optimizer_k, optimizer_snapshot, train_loader, loss_fn, flatten_img=True, c=1):
    model_k.train()
    model_snapshot.train()
    loss = AverageCalculator()
    acc = AverageCalculator()
    ...
    for images, labels in train_loader:
        ...
        optimizer_k.step(optimizer_snapshot.get_param_groups(), c)
...
if __name__ == "__main__":
    ...
    if not args.optimizer in ['SVRGLMD', 'SVRG', 'SVRGLM']:
        raise ValueError("--optimizer must be 'SVRGLMD' or 'SVRG' or 'SVRGLM'.")
    print(args_dict)
    ...
    if args.optimizer == 'SVRG' or args.optimizer == 'SVRGLMD' or args.optimizer == 'SVRGLM':
    ...
    # the optimizer # WILL BE MODIFIED TO INCLUDE SVRG_LM
    ...
    elif args.optimizer == "SVRGLMD":
        optimizer = SVRGLMD_k(model.parameters(), lr=lr, weight_decay=args.weight_decay)
        optimizer_snapshot = SVRGLMD_Snapshot(model_snapshot.parameters())
    elif args.optimizer == "SVRGLM":
        optimizer = SVRGLM_k(model.parameters(), lr=lr, weight_decay=args.weight_decay)
        optimizer_snapshot = SVRGLM_Snapshot(model_snapshot.parameters())


    # output folder 
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    model_name = timestamp + "_" + args.optimizer + "_" + args.nn_model   # + "_c2_f1"
    if args.exp_name != "":
    ...
    for epoch in range(n_epoch):
        t0 = time.time()
        # print("model_snapshot is:", model_snapshot)
        ...
        if args.optimizer == "SVRG":
            ...
            train_loss, train_acc = train_epoch_SVRGLM(model, model_snapshot, optimizer, optimizer_snapshot, train_loader, loss_fn, flatten_img=flatten_img)
        elif args.optimizer == "SVRGLMD":
            train_loss, train_acc = train_epoch_SVRGLMD(model, model_snapshot, optimizer, optimizer_snapshot, train_loader, loss_fn, flatten_img=flatten_img, c=c_init)
        
        # implement for SVRGLMD version 2
        if epoch == 0:
            c_init = train_loss

        # minimum running loss after epoch 0 
        if train_loss < c_init:
            c_init = train_loss
